...
class EasyLevel(AbstractLevel):
    ...
    def from_yaml(cls, loader, node):  # <-- добавим метод класса from_yaml
        ...
        # необходимо выбрать из полученные данных необходимые (словарь)
        obj = cls.Objects()
        obj.config = value
        return {'map': cls.Map(), 'obj': obj}
...
class MediumLevel(AbstractLevel):
    ...
    def from_yaml(cls, loader, node):  # <-- добавим метод класса from_yaml
        # получаем данные из yaml
        value = loader.construct_mapping(node)
        # необходимо выбрать из полученные данных необходимые (данные в виде словаря)
        obj = cls.Objects()
        obj.config = value
        return {'map': cls.Map(), 'obj': obj}
...
print(Levels_yaml)
print(Levels_yaml['levels'][2]['obj'].get_objects(Levels_yaml['levels'][2]['map'].get_map()))