...
class GameSurface(ScreenHandle):
    ...
    def calculate(self, min_x, min_y):
        ...
        screen_size[0] /= self.engine.sprite_size
        screen_size[1] /= self.engine.sprite_size
...
class MiniMap(ScreenHandle):
    ...
    def draw(self, canvas):
        self.fill(colors["wooden"])
        font = pygame.font.SysFont("comicsansms", 20)
        self.blit(font.render('Minimap', True, colors["black"]), (0, 0))
        start_point = (2, 30)
        game_map = self.engine.mini_map
        for i in range(len(game_map)):
            for j in range(len(game_map[i])):
                if game_map[i][j] == 'wall':
                    pygame.draw.rect(self, colors["black"], (start_point[0] + 4 * j, start_point[1] + 4 * i, 4, 4), 0)
                else:
                    pygame.draw.rect(self, colors["wooden"], (start_point[0] + 4 * j, start_point[1] + 4 * i, 4, 4), 0)
        ...
        for obj in objects:
            ...
            if isinstance(obj, Objects.Ally):
                ...
                color = colors['red']
            pygame.draw.rect(self, color, (start_point[0] + 4 * j, start_point[1] + 4 * i, 4, 4), 0)
        j, i = self.engine.hero.position
        pygame.draw.rect(self, colors['yellow'], (start_point[0] + 4 * j, start_point[1] + 4 * i, 4, 4), 0)
